...
> **Repository deployment boundary:** Agent-run setup and verification must use the local anonymous deployment via `CONVEX_AGENT_MODE=anonymous npx convex dev --once`. Do not provision, push to, or mutate a remote dev, preview, or production deployment unless explicitly delegated. Before any remote deployment-affecting command, follow `convex-deploy-guard`.
