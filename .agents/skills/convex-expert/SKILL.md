...
- SELF-VERIFY RULE — before declaring backend work done, run the relevant TypeScript verification and, when Convex runtime/schema validation is useful, use `CONVEX_AGENT_MODE=anonymous npx convex dev --once`. Fix every error it reports before finishing. Do not mutate a remote Convex deployment merely to satisfy verification; remote dev, preview, and production operations require explicit delegation and must follow `convex-deploy-guard`.
