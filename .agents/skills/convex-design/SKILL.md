...
- SELF-VERIFY RULE — before declaring backend work done, run the relevant TypeScript verification. When Convex push/schema validation is useful, prefer the local anonymous backend with `CONVEX_AGENT_MODE=anonymous npx convex dev --once`. Do not push to a remote dev, preview, or production deployment unless explicitly delegated. Any remote deployment-affecting command must first follow `convex-deploy-guard`.
