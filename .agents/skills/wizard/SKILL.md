...
A wizard is ephemeral by default — built for one run, saved to a scratch or `scripts/` path, and deleted when the job is done.

When the user wants a repeatable setup path that should live in the repository, prepare the wizard as a reviewable repository change. Do not commit, push, or publish it unless that Git operation was explicitly delegated.

At handoff, provide the proposed files to preserve and suggested commit wording when useful.
...
- Tell the user how to run it. If it's a repeatable setup path, prepare it for preservation in the repository and recommend linking it from the README so the next person runs the script instead of asking an AI. Committing remains human-owned unless explicitly delegated.
