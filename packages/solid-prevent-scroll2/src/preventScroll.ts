...
import { createEffect, createSignal, createUniqueId, merge, onCleanup } from "solid-js";
import createStyle from "./corvu/create/style";
import { contains } from "./corvu/dom";
import { access, type MaybeAccessor } from "./corvu/reactivity";
import { getScrollAtLocation } from "./corvu/scroll";
import type { Axis } from "./corvu/types";

const [preventScrollStack, setPreventScrollStack] = createSignal<string[]>([]);
const isActive = (id: string) => preventScrollStack().indexOf(id) === preventScrollStack().length - 1;
...
const createPreventScroll = (props: {
  ...
  createEffect(
    ...
    (options) => {
      if (!options.enabled || !options.hideScrollbar) return;
  ...
  function maybePreventTouch(event: TouchEvent) {
    ...
    if (event.touches.length === 2) {
      ...
      if (currentTouchStartAxis == null || currentTouchStartDelta === null) {
        const delta = getTouchXY(event).map((touch, i) => currentTouchStart[i]! - touch) as [number, number];
        const axis = Math.abs(delta[0]) > Math.abs(delta[1]) ? "x" : "y";
...
const getDeltaXY = (event: WheelEvent): [number, number] => [event.deltaX, event.deltaY];

const getTouchXY = (event: TouchEvent): [number, number] =>
  event.changedTouches[0] ? [event.changedTouches[0].clientX, event.changedTouches[0].clientY] : [0, 0];
 ...
 */
const wouldScroll = (target: HTMLElement, axis: Axis, delta: number, wrapper: HTMLElement | null) => {
