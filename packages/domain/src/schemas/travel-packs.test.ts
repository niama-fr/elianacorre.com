...
import { sTravelPackCreate, sTravelPackYoutubeUrl } from "./travel-packs";
import { sSlug } from "./utils";

describe("Travel Pack values", () => {
  it.each(["bali-en-couleurs", "pack-2026", "bali"])("accepts the canonical slug %s", (slug) => {
    expect(S.decodeSync(sSlug)(slug)).toBe(slug);
  });
  ...
  it("preserves raw Markdown descriptions", () => {
    ...
    expect(S.decodeSync(S.String)(description)).toBe(description);
  });
