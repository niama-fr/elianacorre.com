...
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle } from "@ec/ui/components/sheet";
import { Skeleton } from "@ec/ui/components/skeleton";
import { Tooltip, TooltipContent, TooltipTrigger } from "@ec/ui/components/tooltip";
import { useIsMobile } from "@ec/ui/hooks/use-mobile";
...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    if (isMobile) setOpenMobile((isOpen) => !isOpen);
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-lg px-3 py-2 text-left text-sm outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:font-medium [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
