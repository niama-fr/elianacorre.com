...
export function Section(props: SectionProps) {
  const { children, className: C = {}, intent = "default", reverse = false, withGrid = false, ...rest } = props;
  return (
...
export type SectionProps = Omit<React.ComponentProps<"section">, "className"> &
  SectionVariants & { className?: Pick<SectionClass, "base" | "container">; withGrid?: boolean };
...
export function SectionMain(props: SectionMainProps) {
  const { children, className, ...rest } = props;
  return (
    <div {...rest} className={cn(SECTION.main(), className)}>
      {children}
    </div>
  );
}
type SectionMainProps = React.ComponentProps<"div">;

// TITLE -----------------------------------------------------------------------------------------------------------------------------------
export function SectionTitle(props: SectionTitleProps) {
  const { className: C = {}, direction = "column", intent = "primary", title, ...rest } = props;
  ...
  return (
    <h2
      ref={el}
      {...rest}
      className={cn(SECTION.title(), C.title)}
      data-direction={direction}
      data-intent={intent}
      data-visible={visible ? "" : undefined}
    >
      <span className={cn(SECTION.titleRowText(), C.titleRowText)}>{title[0]}</span>
      <div className={cn(SECTION.titleRow(), C.titleRow)}>
        <span className={cn(SECTION.titleRowText(), C.titleRowText, "text-white")}>{title[1]}</span>
