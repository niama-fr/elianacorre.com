...
export const acknowledgeFailedLoopsTask = E.fn(function* (loopsTaskId: Id<"loopsTasks">, now: number) {
  ...
  if (O.isNone(task)) throw new ConvexError("UNKNOWN_LOOPS_TASK");
  if (task.value.status !== "failed") throw new ConvexError("LOOPS_TASK_NOT_FAILED");
  yield* setLoopsTaskAcknowledgedAt(loopsTaskId, now);
});

export const replayFailedLoopsTask = E.fn(function* (
  task: LoopsTasks["Doc"]
): E.fn.Return<WorkflowId, never, DatabaseWriter | MutationCtx> {
...
const sendConfirmationEmail = E.fn(function* ({ profile, task }: SendConfirmationEmailOpts) {
  ...
  const token = yield* createCapabilityToken({ capabilityId: task.newsConfirmationId, secret: env.CAPABILITY_SIGNING_SECRET });
  return yield* E.promise(
    async () =>
      await loops.sendTransactional(ctx, {
        dataVariables: {
          confirmationUrl: getLink({ base: env.SITE_URL, path: "/newsletter/confirmation", token }),
...
const sendEbookEmail = E.fn(function* ({ profile, task }: SendEbookEmailOpts) {
  const ctx = yield* ActionCtx;
  const email = requireProfileEmail(profile);
  const token = yield* createCapabilityToken({ capabilityId: task.ebookDownloadId, secret: env.CAPABILITY_SIGNING_SECRET });
  return yield* E.promise(
    async () =>
      await loops.sendTransactional(ctx, {
        dataVariables: {
          downloadUrl: getLink({ base: env.SITE_URL, path: "/newsletter/ebook", token }),
          firstName: profile.firstName,
        },
        email,
        idempotencyKey: getLoopsTaskDeliveryIdempotencyKey(task),
