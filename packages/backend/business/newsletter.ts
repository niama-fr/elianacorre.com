...
export async function subscribeToNewsletter(ctx: MutationCtx, opts: SubscribeToNewsletterOpts) {
  ...
  const subscriptionId =
    subscription?._id ?? (await createNewsSubscription(ctx, { privacyNoticeId: opts.privacyNoticeId, profileId, requestedAt: now }));
  if (subscription) await patchNewsSubscription(ctx, subscriptionId, { privacyNoticeId: opts.privacyNoticeId, requestedAt: now });
...
async function confirmReactivation(ctx: MutationCtx, { confirmation, now, profileId, subscription }: ConfirmReactivationOpts) {
  ...
  const restored = await resolveEmailDeliveryRestrictionByConfirmation(ctx, { now, restrictionId, restrictionVersion });
...
async function confirmSubscription(ctx: MutationCtx, { confirmation, now, profileId, subscription }: ConfirmSubscriptionOpts) {
  ...
  if (
    ...
    isEmailDeliveryAllowed = await resolveEmailDeliveryRestrictionByConfirmation(ctx, {
      now,
