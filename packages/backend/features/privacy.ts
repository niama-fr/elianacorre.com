...
import { DatabaseWriter, Scheduler } from "../confect/_generated/services";
...
import {
  completePendingPrivacyErasure,
  createPrivacyAuditRequest,
  createPrivacyAuditVerification,
  getPrivacyAudit,
  getPendingPrivacyErasureByEmail,
  listPrivacyAuditsByEmail,
} from "../data/privacy-audits";
import { consumePrivacyGrant, listPrivacyGrantsByEmail, replacePrivacyGrant, revokePrivacyGrant } from "../data/privacy-grants";
import {
  eraseProfileRelationsBatch,
  getProfile,
  getProfileByEmail,
  getProfileIdByEmail,
  hasProfileRelations,
  patchProfile,
  type ProfileErasurePhase,
} from "../data/profiles";
...
// INSPECT ---------------------------------------------------------------------------------------------------------------------------------
export const inspectPrivacySubject = E.fn(function* (email: string, now?: number) {
  const [profile, suppression, audits, grants] = yield* E.all([
    getProfileByEmail(email),
    getNewsSuppressionByEmail(email),
    listPrivacyAuditsByEmail(email),
    listPrivacyGrantsByEmail(email),
  ]);
  const visibleGrants = now === undefined ? grants : grants.filter(({ expiresAt }) => expiresAt > now);
  ...
  if (O.isNone(profile))
    return {
      ...
      privacyState: {
        audits,
        grants: visibleGrants.map(({ expiresAt, requestKind }) => ({ expiresAt, requestKind })),
...
export const processPrivacyErasure = E.fn(function* (email: string) {
  const pendingAudit = yield* getPendingPrivacyErasureByEmail(email);
  if (O.isSome(pendingAudit)) return { outcome: "pending" as const };
  ...
  const privacyAuditId = yield* createPrivacyRequest({ email, kind: "erasure", outcome: "pending", verificationAuditId });
  return {
    erasure: { email, privacyAuditId, profileId: profile.value._id },
    outcome: "pending" as const,
  };
});

export const erasePrivacySubjectBatch = E.fn(function* (opts: {
  email: string;
  phase: ProfileErasurePhase;
  privacyAuditId: Id<"privacyAudits">;
  profileId: Id<"profiles">;
}) {
  return yield* eraseProfileRelationsBatch(opts);
});

export const completePrivacyErasure = E.fn(function* ({
  email,
  privacyAuditId,
  profileId,
}: {
  ...
  const audit = yield* getPrivacyAudit(privacyAuditId);
  if (O.isNone(audit) || audit.value.kind !== "erasure" || audit.value.outcome !== "pending") return true;
  const profile = yield* getProfile(profileId);
  if (yield* hasProfileRelations({ email, profileId })) return false;
  if (O.isSome(profile)) {
    const writer = yield* DatabaseWriter;
    yield* writer.table("profiles").delete(profileId);
  }
  yield* enqueueDeleteContactForPrivacy({ email, privacyAuditId });
  yield* completePendingPrivacyErasure(privacyAuditId);
  return true;
});
...
const processDataRetrieval = E.fn(function* (email: string, kind: "access" | "export") {
  ...
  const data = yield* inspectPrivacySubject(email, Date.now());
...
const recordRequest = E.fn(function* ({ isCompleted, ...create }: RecordRequestOpts) {
  const outcome: PrivacyAudits["FinalOutcome"] = isCompleted ? "completed" : "rejected";
  return {
    outcome,
    privacyAuditId: yield* createPrivacyRequest({ ...create, outcome }),
  };
});
type RecordRequestOpts = DistributiveOmit<PrivacyAudits["RequestCreate"], "outcome" | "performedBy"> & { isCompleted: boolean };

const completeRequest = E.fn(function* (create: DistributiveOmit<PrivacyAudits["RequestCreate"], "outcome" | "performedBy">) {
  return yield* recordRequest({ ...create, isCompleted: true });
});

const rejectRequest = E.fn(function* (create: DistributiveOmit<PrivacyAudits["RequestCreate"], "outcome" | "performedBy">) {
  const { outcome } = yield* recordRequest({ ...create, isCompleted: false });
  return { outcome };
});

const createPrivacyRequest = E.fn(function* (create: DistributiveOmit<PrivacyAudits["RequestCreate"], "performedBy">) {
  const profile = yield* CurrentAdmin;
  return yield* createPrivacyAuditRequest({ ...create, performedBy: profile._id });
});
