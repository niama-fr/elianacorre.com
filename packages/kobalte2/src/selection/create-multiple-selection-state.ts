...
import { access, type MaybeAccessor, mergeDefaultProps } from "@ec/kobalte2/utils";
import { createEffect, createMemo, createSignal } from "solid-js";

import { createControllableSelectionSignal } from "./create-controllable-selection-signal";
import { type MultipleSelection, type MultipleSelectionState, Selection, type SelectionBehavior } from "./types";
 ...
 */
export function createMultipleSelectionState(props: CreateMultipleSelectionStateProps): MultipleSelectionState {
  ...
  const [selectionBehavior, setSelectionBehavior] = createSignal<SelectionBehavior>(access(mergedProps.selectionBehavior)!);
