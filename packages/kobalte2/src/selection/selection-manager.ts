...
import { type MultipleSelectionManager, type MultipleSelectionState, Selection, type SelectionBehavior, type SelectionMode } from "./types";
  ...
  private getAllSelectableKeys() {
    ...
    const addKeys = (key: string | undefined) => {
      while (key != null) {
        ...
        // The loop advances the working key.
        key = this.collection().getKeyAfter(key);
      }
    };
