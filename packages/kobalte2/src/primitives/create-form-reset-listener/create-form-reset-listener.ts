 ...
 */
export function createFormResetListener(element: Accessor<HTMLElement | null | undefined>, handler: () => void) {
  createEffect(
    on(element, (element) => {
      if (element == null) {
        return;
...
function isFormElement(element: HTMLElement): element is HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement {
