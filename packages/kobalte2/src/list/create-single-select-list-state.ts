import { mergeProps, splitProps } from "@ec/kobalte2/utils/solid-compat";
/*
...
import { type CreateListStateProps, createListState, type ListState } from "./create-list-state";
 ...
 */
export function createSingleSelectListState(props: CreateSingleSelectListStateProps): SingleSelectListState {
  ...
  const createListStateProps = mergeProps(defaultCreateListStateProps, {
    ...
    onSelectionChange: (keys: Set<string>) => {
      const key = (keys as Set<string>).values().next().value as string | undefined;
