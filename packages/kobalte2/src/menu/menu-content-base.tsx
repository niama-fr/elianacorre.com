import { createEffect, onMount, splitProps } from "@ec/kobalte2/utils/solid-compat";
...
import { callHandler, composeEventHandlers, contains, mergeDefaultProps, mergeRefs, type Orientation } from "@ec/kobalte2/utils";
import { combineStyle } from "@ec/solid-primitives2/props";
import { type Component, createUniqueId, onCleanup, Show } from "solid-js";
...
import { createFocusScope, type FocusOutsideEvent, type InteractOutsideEvent, type PointerDownOutsideEvent } from "../primitives";
...
export interface MenuContentBaseCommonProps<T extends HTMLElement = HTMLElement> {
  id: string;
...
export type MenuContentBaseProps<T extends ValidComponent | HTMLElement = HTMLElement> = MenuContentBaseOptions &
  Partial<MenuContentBaseCommonProps<ElementOf<T>>>;

export function MenuContentBase<T extends ValidComponent = "div">(props: PolymorphicProps<T, MenuContentBaseProps<T>>) {
  ...
  return (
    <Show when={context.contentPresent()}>
      <Show
        fallback={<Polymorphic<MenuContentBaseRenderProps> as="div" {...context.dataset()} {...commonAttributes} {...others} />}
        when={optionalNavigationMenuContext === undefined || context.parentMenuContext() != null}
      >
        <Popper.Positioner>
