...
import { callHandler, mergeDefaultProps, mergeRefs, type Orientation, scrollIntoViewport } from "@ec/kobalte2/utils";
...
export interface MenuTriggerCommonProps<T extends HTMLElement = HTMLElement> extends Button.ButtonRootCommonProps<T> {
  id: string;
...
export type MenuTriggerProps<T extends ValidComponent | HTMLElement = HTMLElement> = MenuTriggerOptions &
  Partial<MenuTriggerCommonProps<ElementOf<T>>>;

export const MENUBAR_KEYS = {
  next: (dir: Direction, orientation: Orientation) =>
    dir === "ltr" ? (orientation === "horizontal" ? "ArrowRight" : "ArrowDown") : orientation === "horizontal" ? "ArrowLeft" : "ArrowUp",
  previous: (dir: Direction, orientation: Orientation) => MENUBAR_KEYS.next(dir === "ltr" ? "rtl" : "ltr", orientation),
};
 ...
 */
export function MenuTrigger<T extends ValidComponent = "button">(props: PolymorphicProps<T, MenuTriggerProps<T>>) {
  ...
  const onFocus: JSX.EventHandlerUnion<HTMLElement, FocusEvent> = (e) => {
    ...
    if (optionalMenubarContext !== undefined && e.currentTarget.dataset.pointerType !== "touch") optionalMenubarContext.setValue(key);
  };
  ...
  return (
    <Button.Root<Component<Omit<MenuTriggerRenderProps, Exclude<keyof Button.ButtonRootRenderProps, "role">>>>
      ...
      aria-haspopup="true"
      data-highlighted={key() !== undefined && optionalMenubarContext?.value() === key() ? true : undefined}
