...
export interface MenuGroupLabelCommonProps<T extends HTMLElement = HTMLElement> {
  id: string;
}
...
export type MenuGroupLabelProps<T extends ValidComponent | HTMLElement = HTMLElement> = MenuGroupLabelOptions &
  Partial<MenuGroupLabelCommonProps<ElementOf<T>>>;

/**
 * A component used to render the label of a `Menu.Group`.
 * It won't be focusable using arrow keys.
 */
export function MenuGroupLabel<T extends ValidComponent = "span">(props: PolymorphicProps<T, MenuGroupLabelProps<T>>) {
  ...
  return <Polymorphic<MenuGroupLabelRenderProps> aria-hidden="true" as="span" id={local.id} {...others} />;
}
