...
const state = vi.hoisted(() => ({ handlerFetch: vi.fn<(request: Request) => Promise<Response>>() }));

vi.mock(import("@tanstack/react-start/server-entry"), () => ({ default: { fetch: state.handlerFetch } }));
...
describe("cached public app", () => {
  beforeEach(() => state.handlerFetch.mockReset());

  it("applies deterministic security and cache policy to rendered HTML", async () => {
    state.handlerFetch.mockResolvedValue(
      new Response("page", { headers: { "content-type": "text/html", ...CACHE_ROUTE_HEADERS.publicHtml } })
    );
    const app = Object.create(CachedApp.prototype) as CachedApp & { env: { CSP_MODE: string } };
    app.env = { CACHE_REVALIDATION_SECRET: "secret", CSP_MODE: "report-only" };
