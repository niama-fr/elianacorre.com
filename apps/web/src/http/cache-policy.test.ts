...
describe("public response cache policy", () => {
  it("allows declared public HTML to be shared without allowing browser storage", () => {
    const request = new Request("https://elianacorre.com/");
  ...
  it.each(["/newsletter/confirmation", "/newsletter/confirmation/", "/newsletter/ebook", "/newsletter/ebook/"])(
    ...
    (pathname) => {
      const request = new Request(`https://elianacorre.com${pathname}`);
  ...
  it.each(["/newsletter/confirmation", "/newsletter/ebook"])("honors the private route intent at %s", (pathname) => {
    ...
    const sourceResponse = htmlResponse({
  ...
  it("keeps undeclared HTML private", () => {
    const request = new Request("https://elianacorre.com/undeclared");

    const sourceResponse = new Response("<html></html>", {
      headers: {
  ...
  it.each([
    [
      ...
      new Request("https://elianacorre.com/", {
        headers: {
  ...
  it.each(["/robots.txt", "/sitemap.xml"])("preserves declared discovery caching for %s", (pathname) => {
    ...
    const sourceResponse = new Response("discovery", {
      headers: CACHE_ROUTE_HEADERS.publicDiscovery,
    });
  ...
  it.each([
    [
      "POST requests",
      new Request("https://elianacorre.com/_server", {
        method: "POST",
      }),
      htmlResponse(),
    ],
    ...
    [
      "errors",
      new Request("https://elianacorre.com/missing"),
      htmlResponse({
        status: 404,
      }),
    ],
...
describe("public cache gateway", () => {
  it("forwards anonymous reads to the cached entrypoint", () => {
    ...
    expect(
      isPublicCacheCandidate(
        new Request("https://elianacorre.com/robots.txt", {
          method: "HEAD",
        })
      )
    ).toBeTruthy();
  ...
  ])("keeps %s on the uncached entrypoint", (_name, request) => {
