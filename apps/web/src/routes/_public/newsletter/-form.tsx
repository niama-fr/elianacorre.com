import { CURRENT_NEWSLETTER_LEGAL_COPY, zNewsletterSubscriptionValues } from "@ec/domain/schemas/newsletter";
...
export function NewsletterForm() {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        await requestNewsletterSubscription({ data: { ...value, consent: true } });
        form.reset();
        setSubmitted(true);
      } catch {
  ...
  return (
    ...
    >
      <form.AppField name="email" validators={{ onChange: zNewsletterSubscriptionValues.shape.email }}>
      ...
      <form.Field name="consent" validators={{ onChange: zNewsletterSubscriptionValues.shape.consent }}>
        {(field) => {
            ...
            <div className="flex flex-col gap-2">
              <label className="flex items-start gap-3 text-left" htmlFor={field.name}>
                <Input
                  ...
                  type="checkbox"
                />
                <span>{CURRENT_NEWSLETTER_LEGAL_COPY.consent.text}</span>
              </label>
      ...
      <details className="rounded-lg border bg-white/70 p-4 text-left text-sm">
        ...
        <p className="mt-4 whitespace-pre-line">{CURRENT_NEWSLETTER_LEGAL_COPY.privacy.text}</p>
      </details>
