...
The complete manual email-operations procedure, including credentials, DNS, campaigns, alerts, replay, reconciliation, incidents, and legal-text publication, is [`docs/runbooks/newsletter-email-operations.md`](newsletter-email-operations.md).
...
For changes affecting JavaScript delivery, hydration, caching, CSP, redirects, or authentication, use the complete manual two-host staging checklist in [`application-delivery-checks.md`](application-delivery-checks.md).
