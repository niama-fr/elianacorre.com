...
From the repository root, identify the canonical environments in `docs/project-resources.md`. Use `eliana-corre:elianacorre-com-staging` for manual delivery tests; `eliana-corre:elianacorre-com-b1869` is production and requires explicit approval for writes. Then select the backend package and inspect the configured non-production deployment and public URL:
