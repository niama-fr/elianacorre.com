...
- Inventaire des variables dans `docs/runbooks/deployment.md`.
...
La procédure de configuration détaillée et de bascule temporaire se trouve dans `docs/runbooks/deployment.md`, section **Loops webhooks**.
...
Si le fichier est incorrect, restaurer la dernière version approuvée. Les liens renouvelés servent la version publiée actuelle ; conserver l’historique de version et suivre `docs/runbooks/welcome-ebook-recovery.md` pour les accès individuels.
...
Suivre `docs/runbooks/privacy-request-identity-verification.md` pour vérifier l’identité, traiter séparément chaque droit et conserver une trace minimale. Les demandes arrivent à `contact@elianacorre.com`.
...
Suivre `docs/runbooks/newsletter-retention-portability.md`. L’export JSON ou CSV de `/privacy` est fournisseur-indépendant et constitue une donnée personnelle. Le stocker uniquement dans l’espace de migration approuvé et supprimer les copies temporaires.
