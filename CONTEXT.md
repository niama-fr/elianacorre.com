...
**Account**: A Better Auth User used to authenticate one Profile. One Account may collect multiple social-provider identities, but it does not own roles, purchases, memberships, or entitlements. A new member Account maps once to an email-independent `member` Profile even when that Profile has no entitlement. Newsletter subscribers and contact-form correspondents do not receive Accounts merely because an email is known, and linking never changes newsletter consent. _Avoid_: Profile, provider identity, Newsletter subscriber, ownership record

**Authentication Provider**: The replaceable system responsible for provider identities, deciding which provider identities belong to one Account, credentials, external identity assertions, sessions, and account-recovery workflows. Its provider identifiers and email assertions—including hidden, absent, or changing addresses—are authentication/contact data and do not own platform roles, person identity, purchases, memberships, or entitlements. _Avoid_: Profile source, authorization source, ownership source
...
**Content Administrator**: An operator whose Profile is explicitly provisioned with the `admin` role to manage published platform content and subscriber privacy requests. A new Account with the canonical email of that pre-provisioned Profile may be associated with it through any supported Authentication Provider. Provider choice and Workspace-domain membership alone grant no administrative access, and this role is separate from newsletter subscription and member access. _Avoid_: Subscriber, customer account, Google administrator

**Newsletter Consent**: The explicit, revocable permission given by a person to receive the newsletter. It can be confirmed only by the person through the application confirmation link or an authenticated Email Provider preference-center resubscription, never created administratively. A person may have multiple consent periods after unsubscribing and later confirming again; the history records each request, confirmation source, the exact published Privacy Notice presented for the action, and withdrawal rather than only the current subscription state. The invariant consent sentence is repository-owned website copy rather than separately versioned legal evidence. _Avoid_: Account creation, email verification

**Legal Document**: A published or draft French privacy notice. Published privacy notices are historical evidence and must not be changed after publication; future internationalization will be handled as a broader product decision rather than in the current legal document model. The invariant newsletter-consent sentence is static website copy rather than a legal document. _Avoid_: Newsletter-consent legal text, locale-specific copy

**Historical Newsletter Legal Bundle**: The retired pair of newsletter-consent and privacy legal documents formerly referenced by subscriptions. After the privacy-notice backfill, cleanup removed the obsolete references, bundle rows, and newsletter-consent legal texts while preserving the versioned privacy notices. No active application path reads, publishes, or selects a bundle. _Avoid_: Active newsletter evidence, current privacy notice
