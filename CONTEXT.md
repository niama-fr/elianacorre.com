...
**Newsletter Consent**: The explicit, revocable permission given by a person to receive the newsletter. It can be confirmed only by the person through the application confirmation link or an authenticated Email Provider preference-center resubscription, never created administratively. A person may have multiple consent periods after unsubscribing and later confirming again; the history records each request, confirmation source, the legal wording applicable to the action, and withdrawal rather than only the current subscription state. _Avoid_: Account creation, email verification
